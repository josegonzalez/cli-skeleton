...
An example cli tool for the `cli-skeleton` project. It switches the logger to use zerolog for output. Other than configuring zerolog as the logger, it is equivalent to the `hello-world` example.
...
```
Usage: zerolog-logging [--version] [--help] <command> [<args>]
...
To create a new cli tool, the cli-tool will need to be initialized. This tool will be called `zerolog-logging`:

```shell
mkdir zerolog-logging
go mod init zerolog-logging
```
...
func Run(args []string) int {
  ...
  commandMeta.Ui = command.ZerologUiWithFields(commandMeta.Ui, make(map[string]interface{}, 0))
...
Run `go build -ldflags "-X main.Version=0.1.0"` to build the `0.1.0` version of the tool in the current directory. Running `./zerolog-logging` will now show the following output:
...
The `cli-skeleton` project includes a helpful `version` command that can be executed via `./zerolog-logging version` with the following output:

```
5:55PM INF 0.1.0
```

Note that the output here includes a timestamp and log level. This formatting is introduced by the Zerolog logger, and specifically by the following line:
...
Adding a new subcommand is straightforward. For the example `zerolog-logging` app, an `eat` command will be created. To start, create a `commands` directory that contains an `eat.go` file. This file should contain an `EatCommand` struct as follows:
